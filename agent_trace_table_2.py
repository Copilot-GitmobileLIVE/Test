...
write a results CSV with the real answer and tools called.

Example:
    python agent_trace_table_2.py --test-data test_data.csv --agent-name "Web Q&A Agent"
    python agent_trace_table_2.py --test-data test_data.csv  # uses Web Q&A Agent by default
"""
    ...
    for i, row in enumerate(rows, 1):
        ...
        with out_path.open("w", newline="", encoding="utf-8") as fh:
            writer = csv.DictWriter(
                fh,
                fieldnames=["question", "expected_answer", "real_answer", "tools_called"],
            )
            writer.writeheader()
